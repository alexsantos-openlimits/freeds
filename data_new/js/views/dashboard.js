...
import { fmtWatts, fmtKwh, fmtNumber, fmtUptime, el } from "../components/dom.js";
import { toast } from "../components/toast.js";

export default async function mount(container, ctx) {
  const { i18n } = ctx;
  ctx.setTitle(i18n.t("dashboard.title"));
  ...
  function buildSkeleton(status) {
    ...
    const relayCount = (status.relays || []).length || 4;

    return `
      <div class="view-header">
        <h2 data-i18n="dashboard.title"></h2>
        <p data-i18n="dashboard.subtitle_unused" id="dash-updated"></p>
      </div>
